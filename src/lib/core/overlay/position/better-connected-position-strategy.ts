...
export class BetterConnectedPositionStrategy implements PositionStrategy {
  ...
  apply(element: HTMLElement): void {
    ...
    if (this._canPush) {
      ...
      this._applyPosition(element, fallback!.pos, pushedPoint, fallback!.originPoint, viewportRect);
      return;
    }
    ...
    this._applyPosition(
        element, fallback!.pos, fallback!.overlayPoint, fallback!.originPoint, viewportRect);
  }
   ...
   * @param overlayRect The CLientRect for the overlay element.
   */
  ...
  private _setFlexWrapperStyles(
    ...
    if (position.overlayY === 'top') {
      ...
      style.height = `${viewport.bottom - originPoint.y}px`;
      style.top = `${originPoint.y}px`;
    } else if (position.overlayY === 'bottom') {
      // Overlay is opening "upward"
      style.height = `${originPoint.y - viewport.top}px`;
      style.top = `${viewport.top}px`;
    } else {
      // ??? ugh centering
    }
    ...
    if (isBoundedByLeftViewportEdge) {
      style.left = `${viewport.left}px`;
      style.width = `${originPoint.x - viewport.left}px`;
    } else if (isBoundedByRightViewportEdge) {
      style.left = `${originPoint.x}px`;
      style.width = `${viewport.right - originPoint.x}px`;
    } else {
      // ???
    }
